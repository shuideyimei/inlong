...
public class BaseMetricReporter {
    ...
    private List<AuditDataVo> generateTestAuditDataVo(int dataCount, double apiDiscardRate, double apiToAgentLossRate,
        ...
        for (int i = 0; i < dataCount; i++) {
            ...
            AuditDataVo apiReceive = createAuditDataVo(ip, dockerId, threadId, logTs, groupId, streamId, "1", count,
                    size, random.nextInt(201));
            result.add(apiReceive);

            if (random.nextDouble() >= apiDiscardRate) {
                AuditDataVo apiSend = createAuditDataVo(ip, dockerId, threadId, logTs, groupId, streamId, "2", count,
                        size, random.nextInt(201));
                result.add(apiSend);

                if (random.nextDouble() >= apiToAgentLossRate) {
                    AuditDataVo agentReceive = createAuditDataVo(ip, dockerId, threadId, logTs, groupId, streamId, "3",
                            count, size, random.nextInt(201));
                    result.add(agentReceive);

                    if (random.nextDouble() >= agentDiscardRate) {
                        AuditDataVo agentSend = createAuditDataVo(ip, dockerId, threadId, logTs, groupId, streamId, "4",
                                count, size, random.nextInt(201));
                        result.add(agentSend);

                        if (random.nextDouble() >= agentToDataProxyLossRate) {
                            AuditDataVo dataProxyReceive = createAuditDataVo(ip, dockerId, threadId, logTs, groupId,
                                    streamId, "5", count, size, random.nextInt(201));
                            result.add(dataProxyReceive);

                            if (random.nextDouble() >= dataProxyDiscardRate) {
                                AuditDataVo dataProxySend = createAuditDataVo(ip, dockerId, threadId, logTs, groupId,
                                        streamId, "6", count, size, random.nextInt(201));
