...
PID=$(ps -ef | grep "audit-tool" | grep -v grep | awk '{ print $2}')
...
SERVER_JAR=$(ls -lt "${BASE_DIR}"/lib |grep audit-tool | head -2 | tail -1 | awk '{print $NF}')

nohup "$JAVA" $JAVA_OPTS -Daudit.log.path="$LOG_DIR" -Dloader.path="$BASE_DIR/conf,$BASE_DIR/lib/" -jar "$BASE_DIR/lib/$SERVER_JAR"  1>"${LOG_DIR}"/tool.log  2>"${LOG_DIR}"/tool-error.log &
